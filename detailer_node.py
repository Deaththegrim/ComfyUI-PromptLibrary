...
Self-contained one-node face/eyes/hands/feet detailer. No Comfy custom-node
...
Pipeline per target (face → eyes → hands → feet):
...
Optimisations baked in:
  ...
  - Wildcard cache: per-target CLIPTextEncode runs once per target, not once
    per detected bbox.
  - Per-target overrides: face_/eyes_/hands_/feet_threshold and _denoise floats
...
Outputs:
  ...
  - detections_preview: input image with colored bboxes per target (face=green,
    eyes=cyan, hands=magenta, feet=violet) — wire to a SaveImage to debug.
"""
...
# Coarse-to-fine. Face refines on the head; eyes polish within the
# already-refined face; hands and feet are independent extremities last
# (VRAM peaks there, so we run them after the face work is committed).
# Skin and mouth passes were removed: skin overlapped face on most
# workflows, and no public mouth/teeth detector reliably finds the
# actual mouth on stylized art — face-pass crop at 3.0x covers it.
_TARGET_ORDER = ("face", "eyes", "hands", "feet")
...
def _refine_to_blend_mask(
    ...
    or feet crop_factor=2.5, the crop extends well into the background, and
...
# Only the chars we need: target labels (face/eyes/hands/feet) + digits + "."
_FONT_5x7 = {
...
def _enhance_one_pass(
    ...
    drop_size: int, min_detail_size: int, force_inpaint: bool,
    ...
    # whole detail() call. Face + eyes-fallback-to-face passes can share
    # bboxes — SAM result is identical, so we cache by raw_bbox tuple.
    needs_sam = []
    if sam_ready:
    ...
    for i, (raw_bbox, _conf) in enumerate(plan.detections):
        ...
        # Geometry gate: composite pastes back at raw bbox resolution, so a
        # face <48 px ends up <48 px no matter what guide_size is. Skip those
        # to avoid burning sample time on detail that will be averaged away.
        rx1, ry1, rx2, ry2 = raw_bbox
        raw_short = min(rx2 - rx1, ry2 - ry1)
        if min_detail_size > 0 and raw_short < min_detail_size:
            skipped_too_small += 1
    ...
    if skipped_too_small:
        logging.info(
            "[SmartDetailer] %s: skipped %d bbox(es) below min_detail_size=%dpx — "
            "composite would lose new detail at this resolution.",
            plan.name, skipped_too_small, min_detail_size)
...
class GrimmRibbitySmartDetailer:
    """One-node face/eyes/hands/feet detailer with SAM mask refinement."""
        ...
        def stp(target):
            return ("INT", {"default": 0, "min": 0, "max": 200,
                "tooltip": (
                    ...
                    f"Useful for cheap eyes passes (10-15 steps) paired with a "
        ...
        return {
            "required": {
                ...
                "bbox_face":  (bbox_models, {"default": _NONE, "tooltip":
                    "YOLO bbox model for the face / (fallback) eyes passes. "
                    ...
                    "enable_face / enable_eyes is True."}),
                ...
                "denoise": ("FLOAT", {"default": 0.45, "min": 0.01, "max": 1.0, "step": 0.01,
                    ...
                    "ride on top: face/eyes use this value directly, hands run +0.05. "
                    "Higher = more aggressive redraw (can change identity); lower = more "
                ...
                "mask_strength": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 2.0, "step": 0.05,
                    ...
                    "(softer face refinement that keeps more of the original). 1.2-1.5 = harder edge "
            ...
            "optional": {
                ...
                "feet_max":        maxn("feet"),
                "feet_steps":      stp("feet"),
                # Per-target crop_factor overrides
                ...
                # Per-target cycle counts — under the crop row. Cycle 2+ re-runs
                # the same enabled pass on the previous cycle's output using the
                # SAME cached detections and SAM masks (no re-detection cost).
                # Use when a single denoise-0.4 pass barely changes the output.
                "face_cycles":  ("INT", {"default": 1, "min": 1, "max": 8, "tooltip":
                    "Number of face refinement cycles. Each cycle re-samples on the previous "
                    "output with a seed bump. 2-3 cycles compound detail when one pass barely "
                    "moves the result. Linear cost in time, no extra VRAM."}),
                "eyes_cycles":  ("INT", {"default": 1, "min": 1, "max": 8, "tooltip":
                    "Number of eye refinement cycles. See face_cycles."}),
                "hands_cycles": ("INT", {"default": 1, "min": 1, "max": 8, "tooltip":
                    "Number of hand refinement cycles. See face_cycles."}),
                "feet_cycles":  ("INT", {"default": 1, "min": 1, "max": 8, "tooltip":
                    "Number of feet refinement cycles. See face_cycles."}),
                ...
                "min_detail_size": ("INT", {"default": 48, "min": 0, "max": 1024, "tooltip":
                    "Per-bbox geometry gate (px). After detection, any raw bbox whose short "
                    "side is below this is SKIPPED with a log line instead of sampled. Reason: "
                    "the detailer composites the sampled crop back at the raw bbox resolution, "
                    "so a 40-px face still ends up 40 px in the output regardless of guide_size "
                    "— bilinear downsample averages away the new detail. Default 48 catches "
                    "the common 'face is tiny in a full-body shot' case. Set 0 to disable."}),
    ...
    OUTPUT_TOOLTIPS = (
        ...
        "passes — face + eyes + hands + feet, OR'd together. Use this if you "
        ...
        "green, eyes = cyan, hands = magenta, feet = violet. Bbox color "
    ...
    def detail(self, image, model, clip, vae, positive, negative,
               ...
               face_cycles=1, eyes_cycles=1, hands_cycles=1, feet_cycles=1,
        ...
        # denoise_override, max_override). Order is face → eyes → hands → feet.
        ovr = {
        ...
        cycles_pick = {
            "face":  max(1, int(face_cycles)),
            "eyes":  max(1, int(eyes_cycles)),
            "hands": max(1, int(hands_cycles)),
            "feet":  max(1, int(feet_cycles)),
        }
        ...
        # Keeps eyes-on-face from re-running the face detector when the eyes
        # pass falls back to bbox_face.
        ...
        total_detections = sum(len(p.detections) * cycles_pick[p.name] for p in passes)
        for p in passes:
        ...
        if _PROGRESS_BAR_CLS is None:
            _resolve_comfy_helpers()
        pbar = _PROGRESS_BAR_CLS(total_detections, node_id=unique_id) if _PROGRESS_BAR_CLS else None
        ...
        # SAM mask cache shared across passes — when face + eyes-fallback
        # share bboxes, SAM runs once total instead of once each.
        sam_mask_cache: dict[tuple[int, int, int, int], torch.Tensor | None] = {}
        ...
        for i, p in enumerate(passes):
            ...
            n_cycles = cycles_pick[p.name]
            cycles_tag = f" cycles={n_cycles}" if n_cycles > 1 else ""
            logging.info(
                "[SmartDetailer] >>> %s pass starting (%d bbox, denoise=%.2f crop=%.2f"
                " threshold=%.2f%s) wc='%s'",
                p.name.upper(), len(p.detections), p.denoise, preset["crop_factor"],
                p.threshold, cycles_tag, wildcard_text)
            try:
                if torch.cuda.is_available():
            ...
            for cyc in range(n_cycles):
                if n_cycles > 1:
                    logging.info("[SmartDetailer]     %s cycle %d/%d",
                                  p.name, cyc + 1, n_cycles)
                # Per-cycle seed bump so each pass adds fresh noise instead of
                # re-sampling the same trajectory. Cached p.detections and
                # sam_mask_cache mean no re-detection cost.
                cycle_seed = int(seed) + i * 1000 + cyc * 137
                running, mask = _enhance_one_pass(
                    running, model, clip, vae, positive, negative, p, sam_handle,
                    img_uint8_np=img_uint8_np,
                    seed=cycle_seed, steps=int(steps), cfg=float(cfg),
                    sampler_name=sampler_name, scheduler=scheduler,
                    guide_size=int(guide_size), max_size=int(max_size),
                    drop_size=int(drop_size), min_detail_size=int(min_detail_size),
